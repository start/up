...
SPOILER
  After beating the Elite Four, Blue steals a Red Delicious from Red.`
      ...
      expect(html).to.equal(
        ...
        + '<input id="article-revealable-1" type="checkbox">'
        + '<label for="article-revealable-1" role="button" tabindex="0">reveal</label>'
        + '<div role="alert">'
  ...
  context('The parseAndRenderInline method', () => {
    ...
    specify('can be used with rendering settings', () => {
      ...
      expect(html).to.equal(
        ...
        + '<label for="reply-104-revealable-1" role="button" tabindex="0">reveal</label>'
        + '<span role="alert">'
        + 'Blue steals a Red Delicious from Red.'
        + '</span>'
        + '</span>')
