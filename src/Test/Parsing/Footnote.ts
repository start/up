...
describe('In a paragraph, parenthesized text starting with a caret', () => {
  it("produces a footnote node inside the paragraph, and a footnote block node for the footnote after the paragraph", () => {
    ...
    expect(Up.parseDocument(footnoteProducedByParentheses)).to.deep.equal(
      new UpDocument([
        new Paragraph([
...
describe('Square bracketed text starting with a caret', () => {
  it("also produces a footnote node", () => {
    ...
    expect(Up.parseDocument(footnoteProducedByParentheses)).to.deep.equal(
      Up.parseDocument(footnoteProducedBySquareBrackets))
  })
})
...
describe('A footnote', () => {
  it('is evaluated for inline conventions', () => {
    ...
    expect(Up.parseDocument("I don't eat cereal. (^Well, I *do*, but I pretend not to.) Never have.")).to.deep.equal(
      new UpDocument([
        new Paragraph([
          new PlainText("I don't eat cereal."),
          footnote,
