...
context('A linkified footnote can have whitespace between itself and its bracketed URL, but only if the URL satisfies one of the following conditions:', () => {
  ...
  context('When the URL has a scheme', () => {
    ...
    specify('the URL must not contain any spaces', () => {
      ...
      expect(Up.parseDocument('(^the phone was dead) (https://stackoverflow.com is where I learned)')).to.deep.equal(
        new UpDocument([
          new Paragraph([
            footnote,
            new PlainText(' '),
  ...
  describe('When the URL starts with a slash, the URL', () => {
    it('must not contain any spaces', () => {
      ...
      expect(Up.parseDocument('(^the phone was dead) (/r9k/ was talking about it)')).to.deep.equal(
        new UpDocument([
          new Paragraph([
            footnote,
            new PlainText(' '),
    ...
    it('must not have its slash escaped', () => {
      ...
      expect(Up.parseDocument('(^slash) (\\/r9k/)')).to.deep.equal(
        new UpDocument([
          new Paragraph([
            footnote,
            new PlainText(' '),
  ...
  describe('When the URL starts with a hash mark ("#"), the URL', () => {
    ...
    it('must not contain any spaces', () => {
      ...
      expect(Up.parseDocument('(^the game was dead) (#starcraft2 was never trending)')).to.deep.equal(
        new UpDocument([
          new Paragraph([
            footnote,
            new PlainText(' '),
    ...
    it('must not have its hashmark escaped', () => {
      ...
      expect(Up.parseDocument('(^hash mark) (\\#starcraft2)')).to.deep.equal(
        new UpDocument([
          new Paragraph([
            footnote,
            new PlainText(' '),
  ...
  specify('If none of the conditions are satisfied, the footnote is not linkified', () => {
    ...
    expect(Up.parseDocument('(^the phone was dead) (really)')).to.deep.equal(
      new UpDocument([
        new Paragraph([
          footnote,
          new PlainText(' '),
