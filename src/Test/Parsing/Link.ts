...
describe('An otherwise-valid link with its URL escaped', () => {
  it('does not produce a link node', () => {
    expect(Up.parseDocument('[call me](\\tel:5555555555)')).to.deep.equal(
      insideDocumentAndParagraph([
...
describe("A link's contents", () => {
  ...
  it('is evaluated for other links', () => {
    expect(Up.parseDocument('[Google is probably not [Bing][https://bing.com]][https://google.com].')).to.deep.equal(
      insideDocumentAndParagraph([
        new Link([
  ...
  it('is evaluated for bare URLs', () => {
    expect(Up.parseDocument('[Google is not at https://bing.com][https://google.com].')).to.deep.equal(
      insideDocumentAndParagraph([
        new Link([
...
describe("A link's URL", () => {
  it('is not evaluated for other conventions', () => {
    expect(Up.parseDocument('I like [this site][https://stackoverflow.com/?search=*hello*there].')).to.deep.equal(
      insideDocumentAndParagraph([
        new PlainText('I like '),
        new Link([
          new PlainText('this site')
...
describe('A link produced by square brackets', () => {
  it('can start with square bracketed text', () => {
    expect(Up.parseDocument('I like [[only one] site][https://stackoverflow.com].')).to.deep.equal(
      insideDocumentAndParagraph([
        new PlainText('I like '),
        new Link([
          new SquareParenthetical([
...
describe("The URL of a link produced by square brackets", () => {
  it('can contain matching unescaped brackets', () => {
    expect(Up.parseDocument('Here is a [strange URL][https://google.com/search?q=[hi]].')).to.deep.equal(
      insideDocumentAndParagraph([
  ...
  it('can have an escaped, unmatched closing bracket', () => {
    expect(Up.parseDocument('I like [this site][https://google.com/?fake=\\]query]. I bet you do, too.')).to.deep.equal(
      insideDocumentAndParagraph([
        new PlainText('I like '),
        new Link([
          new PlainText('this site')
