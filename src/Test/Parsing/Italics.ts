...
describe('Text surrounded by single underscores', () => {
  it('is put inside an italic node', () => {
    expect(Up.parseDocument('Hello, _world_!!')).to.deep.equal(
...
describe('Italicized text', () => {
  it('is evaluated for inline conventions', () => {
    expect(Up.parseDocument('Hello, _`world`_!')).to.deep.equal(
      insideDocumentAndParagraph([
        new PlainText('Hello, '),
        new Italic([
  ...
  it('can contain further italicized text', () => {
    expect(Up.parseDocument('Hello, _my _little_ world_!')).to.deep.equal(
      insideDocumentAndParagraph([
        new PlainText('Hello, '),
        new Italic([
          new PlainText('my '),
  ...
  it('can contain stressed text', () => {
    expect(Up.parseDocument('Hello, _my __little__ world_!')).to.deep.equal(
      insideDocumentAndParagraph([
        new PlainText('Hello, '),
        new Italic([
          new PlainText('my '),
...
describe('Double underscores followed by two separate single closing underscores', () => {
  it('produces 2 nested italic nodes', () => {
    expect(Up.parseDocument('__Warning:_ never feed this tarantula_')).to.deep.equal(
      insideDocumentAndParagraph([
        new Italic([
          new Italic([
...
describe('Text separated from (otherwise surrounding) underscores by whitespace', () => {
  it('is not put inside an italic node', () => {
    expect(Up.parseDocument('Birdie Sanders _ won _ Wisconsin')).to.deep.equal(
      insideDocumentAndParagraph([
