...
describe('A single line bulleted by an integer followed by a period', () => {
  it('does not produce an ordered list', () => {
    expect(Up.parseDocument('1783. Not a good year for Great Britain.')).to.deep.equal(
      new UpDocument([
        new Paragraph([
...
describe('A single line bulleted by a number sign followed by a period', () => {
  it('produces an ordered list node containing ordered list item nodes', () => {
    expect(Up.parseDocument('#. Hello, Lavender Town!')).to.deep.equal(
      new UpDocument([
        new OrderedList([
          new OrderedList.Item([
            new Paragraph([
...
describe('A single line bulleted by a number sign followed by a closing parenthesis', () => {
  it('produces an ordered list node containing ordered list item nodes', () => {
    expect(Up.parseDocument('#) Hello, Celadon City!')).to.deep.equal(
      new UpDocument([
        new OrderedList([
          new OrderedList.Item([
            new Paragraph([
...
describe('A single line bulleted by an integer followed by a closing parenthesis', () => {
  it('produces an ordered list node containing an ordered list item node with an explicit ordinal', () => {
    expect(Up.parseDocument('1) Hello, Celadon City!')).to.deep.equal(
      new UpDocument([
        new OrderedList([
          new OrderedList.Item([
            new Paragraph([
...
# Goodbye, world!
  ===============`
    expect(Up.parseDocument(withSeparation)).to.deep.equal(Up.parseDocument(withoutSeparation))
  })
...
\tPepe, PA 17101
  ...
  I used to live there.`
    ...
    expect(Up.parseDocument(withMixedIndentation)).to.deep.equal(
      new UpDocument([
        new OrderedList([
          new OrderedList.Item([
            new LineBlock([
