...
context('A plus sign followed by a hyphen normally produces a plus-minus sign', () => {
  context('This applies within regular text:', () => {
    specify('Between words', () => {
      expect(Up.parseDocument("Yeah, it uses base HP+-4.")).to.deep.equal(
    ...
    specify('Following a word', () => {
      expect(Up.parseDocument("I have 10+- ...")).to.deep.equal(
        insideDocumentAndParagraph([
    ...
    specify('Preceding a word', () => {
      expect(Up.parseDocument('I have three homes, +-two.')).to.deep.equal(
        insideDocumentAndParagraph([
    ...
    specify('Surrounded by whitespace', () => {
      expect(Up.parseDocument("Well, +- a million.")).to.deep.equal(
        insideDocumentAndParagraph([
  ...
  context('This does not apply within:', () => {
    ...
    specify('Inline code', () => {
      expect(Up.parseDocument("`x+-y`")).to.deep.equal(
        insideDocumentAndParagraph([
          new InlineCode('x+-y')
        ]))
    })
