...
context('Square bracketed text can be directly followed by whitespace followed by', () => {
  context('an inline spoiler', () => {
    specify('that only contains whitespace directly after the colon', () => {
      expect(Up.parseDocument('After you beat the Elite Four, you have to face [the one and only] [SPOILER: Gary].')).to.deep.equal(
    ...
    specify('that contains whitespace, but non directly after the colon', () => {
      expect(Up.parseDocument('After you beat the Elite Four, you have to face [the one and only] [SPOILER:Gary Oak].')).to.deep.equal(
        insideDocumentAndParagraph([
          new PlainText('After you beat the Elite Four, you have to face '),
          new SquareParenthetical([
            new PlainText('[the one and only]')
  ...
  context('an image', () => {
    specify('that only contains whitespace directly after the colon', () => {
      expect(Up.parseDocument('After you beat the Elite Four, you have to face Gary [in Pokémon Red/Blue/Yellow] [image: Gary] (example.com/gary.png).')).to.deep.equal(
        insideDocumentAndParagraph([
    ...
    specify('that contains whitespace, but non directly after the colon', () => {
      expect(Up.parseDocument('After you beat the Elite Four, you have to face Gary [in Pokémon Red/Blue/Yellow] [image:Gary Oak] (example.com/gary.png).')).to.deep.equal(
        insideDocumentAndParagraph([
          new PlainText('After you beat the Elite Four, you have to face Gary '),
          new SquareParenthetical([
            new PlainText('[in Pokémon Red/Blue/Yellow]')
