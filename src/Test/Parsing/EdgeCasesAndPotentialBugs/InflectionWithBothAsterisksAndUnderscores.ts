...
describe('Emphasis', () => {
  it('cannot be closed by an underscore', () => {
    expect(Up.parseDocument('Xamarin is now *free_!')).to.deep.equal(
...
describe('Italics', () => {
  it('cannot be closed by an asterisk', () => {
    expect(Up.parseDocument('Xamarin is now _free*!')).to.deep.equal(
      insideDocumentAndParagraph([
...
describe('Text surrounded by an underscore and an asterisk on each side', () => {
  it('is italicized and emphasized', () => {
    expect(Up.parseDocument('Koopas! _*Mario is on his way!*_ Grab your shells!')).to.deep.equal(
      insideDocumentAndParagraph([
...
describe('Text surrounded by double asterisk and double underscores on each side', () => {
  it('is stressed and bold', () => {
    expect(Up.parseDocument('Koopas! **__Mario is on his way!__** Grab your shells!')).to.deep.equal(
      insideDocumentAndParagraph([
        new PlainText('Koopas! '),
        new Stress([
          new Bold([
