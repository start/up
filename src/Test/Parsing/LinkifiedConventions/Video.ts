...
describe('A video directly followed by an inline NSFW convention', () => {
  it('is not linkified', () => {
    expect(Up.toDocument('After you beat the Elite Four, [video: you fight Gary](https://example.com/fight.webm)[NSFW: and win].')).to.deep.equal(
      insideDocumentAndParagraph([
        new PlainText('After you beat the Elite Four, '),
        new Video('you fight Gary', 'https://example.com/fight.webm'),
        new InlineNsfw([
...
describe('A video directly followed by an inline NSFL convention', () => {
  it('is not linkified', () => {
    expect(Up.toDocument('After you beat the Elite Four, [video: you fight Gary](https://example.com/fight.webm)[NSFL: and win].')).to.deep.equal(
      insideDocumentAndParagraph([
        new PlainText('After you beat the Elite Four, '),
        new Video('you fight Gary', 'https://example.com/fight.webm'),
        new InlineNsfl([
...
describe('An otherwise-valid linkified video convention with its linkifying URL escaped', () => {
  it('is not linkified', () => {
    expect(Up.toDocument('[video: phone call](https://example.com/phonecall.webm)(\\tel:5555555555)')).to.deep.equal(
      insideDocumentAndParagraph([
...
context("When an otherwise-valid linkified video's URL starts with whitespace, and the first character in the actual URL is escaped,", () => {
  specify('the video convention is not linkified', () => {
    expect(Up.toDocument('[video: phone call](https://example.com/phonecall.ogg)( \t \\tel:5555555555)')).to.deep.equal(
      insideDocumentAndParagraph([
