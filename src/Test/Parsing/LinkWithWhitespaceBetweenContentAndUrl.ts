...
context('A link can have whitespace between its bracketed content and bracketed URL, but only if the URL satisfies one of the following conditions:', () => {
  ...
  describe('When the URL has a scheme', () => {
    ...
    specify('the URL must not contain any spaces', () => {
      expect(Up.toDocument('[agreed] (https://stackoverflow.com is nice)')).to.deep.equal(
  ...
  describe('When the URL starts with a slash, the URL', () => {
    it('must not contain any spaces', () => {
      expect(Up.toDocument('[yeah] (/r9k/ inspires geniune pity)')).to.deep.equal(
        insideDocumentAndParagraph([
          new SquareParenthetical([
            new PlainText('[yeah]')
          ]),
    ...
    it('must not have its slash escaped', () => {
      expect(Up.toDocument('[yeah] (\\/r9k/)')).to.deep.equal(
        insideDocumentAndParagraph([
          new SquareParenthetical([
            new PlainText('[yeah]')
          ]),
  ...
  describe('When the URL starts with a hash mark ("#"), the URL', () => {
    ...
    it('must not contain any spaces', () => {
      expect(Up.toDocument('[yeah] (#starcraft2 was never trending)')).to.deep.equal(
        insideDocumentAndParagraph([
          new SquareParenthetical([
            new PlainText('[yeah]')
          ]),
    ...
    it('must not have its hashmark escaped', () => {
      expect(Up.toDocument('[yeah] (\\#starcraft2)')).to.deep.equal(
        insideDocumentAndParagraph([
          new SquareParenthetical([
            new PlainText('[yeah]')
          ]),
  ...
  specify('If none of the conditions are satisfied, no link node is produced', () => {
    expect(Up.toDocument('[no] (really)')).to.deep.equal(
      insideDocumentAndParagraph([
        new SquareParenthetical([
          new PlainText('[no]')
        ]),
