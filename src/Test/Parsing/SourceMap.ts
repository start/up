...
context('When the "createSourceMap" config setting is not set to true', () => {
  specify('no source maps are produced', () => {
    expect(Up.parseDocument("I enjoy apples.")).to.deep.equal(
      new UpDocument([
...
context('When the "createSourceMap" config setting is set to true, outline nodes are given a source line number.', () => {
  ...
  specify("The source line numbers start at 1.", () => {
    expect(up.parseDocument('Hi!')).to.deep.equal(
      new UpDocument([
...
0;      false;  false`
    ...
    context('Media nodes are given a source line number if they were "outlined":', () => {
      specify('Audio nodes', () => {
        expect(up.parseDocument('[audio: haunted house] (example.com/hauntedhouse.ogg)')).to.deep.equal(
          new UpDocument([
      ...
      specify('Images', () => {
        expect(up.parseDocument('[image: haunted house] (example.com/hauntedhouse.svg)')).to.deep.equal(
          new UpDocument([
      ...
      specify('Videos', () => {
        expect(up.parseDocument('[video: haunted house] (example.com/hauntedhouse.webm)')).to.deep.equal(
          new UpDocument([
    ...
    describe('A link containing an "outlined" media node', () => {
      it('is given a source line number (but the media node it contains is not)', () => {
        expect(up.parseDocument('[image: haunted house] (example.com/hauntedhouse.svg) (example.com/gallery)')).to.deep.equal(
          new UpDocument([
            new Link([
...
I do eat apples, though.`
    ...
    expect(Up.parseDocument(markup, { createSourceMap: true })).to.deep.equal(
      new UpDocument([
        new Paragraph([
