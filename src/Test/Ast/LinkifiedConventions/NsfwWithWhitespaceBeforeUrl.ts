...
context('A linkified NSFW convention can have whitespace between itself and its bracketed URL under certain conditions.', () => {

  context('If the URL does not have a scheme, does not start with a slash, or does not start with a hash mark ("#")', () => {
    specify('we assume the author did not indent to produce a link, so no NSFW convention node is not linkfieds', () => {
