...
#
# We also have export unit tests, which describe how the Up library is exported. For more information, see
# the `report` target.
...
#
# On the other hand, our handful of export unit tests are run against `npm_publish_dir`. If we were to have
# istanbul run our export tests, we'd get an unhelpful test coverage summary, because istanbul doesn't realize
# that `npm_publish_dir` is copied from `compiled_dir`. Therefor, at least for now, istanbul only runs our
# behavioral tests.
